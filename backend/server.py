...
DOCUMENT_PATTERNS = {
    ...
    "Invoice": {
        "keywords": [
            "invoice", "invoice no", "bill to", "amount due", "total",
    ...
    "Contract": {
        "keywords": [
            "contract", "agreement", "party a", "party b", "terms",
  ...
  "Report": {
    "keywords": [
        "report", "site report", "project details", "project report",
...
def extract_text_from_shape(shape, texts):
    ...
    if getattr(shape, "has_text_frame", False):
        texts.append(shape.text_frame.text)  # type: ignore[attr-defined]

    # 2. Grouped shapes — recurse inside
    if shape.shape_type == MSO_SHAPE_TYPE.GROUP:
        for subshape in shape.shapes:  # type: ignore[attr-defined]
...
def is_email(text: str, debug: bool = False) -> bool:
    """
    Conservative email detector returning True only when there's strong evidence.
    If debug=True, it prints diagnostic scores.
    """
    t = (text or "").lower()
    ...
    # Presence of long plain paragraphs (emails usually have blocks of prose)
    paragraphs = [p.strip() for p in t.split("\n\n") if p.strip()]
    long_paragraphs = sum(1 for p in paragraphs if len(p.split()) > 20)

    # Resume-like indicators (should *decrease* the likelihood of email)
    resume_indicators = [
    ...
    # Scoring — tuned to avoid false positives on resumes
    score = 0.0
    score += min(header_count, 3) * 1.0        # headers are strong
    score += min(email_count, 3) * 0.6         # email addresses contribute
    score += 1.0 if has_greeting else 0.0
    score += 1.0 if has_signoff else 0.0
    score += min(long_paragraphs, 2) * 0.8

    # Subtract points for resume-like content
    ...
    if debug:
        print("EMAIL DEBUG:",
              ...
              f"greeting={has_greeting}",
              f"signoff={has_signoff}",
              f"long_paras={long_paragraphs}",
              f"resume_matches={resume_matches}",
              f"score={score:.2f}")

    # Decision threshold — only very confident => True
    return score >= 2.5
...
def semantic_score(text, label):
    ...
    if label not in examples:
        return 0.0
    try:
        emb1 = model.encode([text[:400]])
        emb2 = model.encode([examples[label]])
...
def classify(text: str):
    ...
    # --- Conservative email detection (after resume check)
    # But we will run resume BEFORE checking email

    # -------------------------
    # 1) RESUME HIGH PRIORITY CHECK
    # -------------------------
    ...
    if resume_score >= 0.55:   # strong threshold to avoid email false positives
        return "Resume", "Strong resume patterns detected", min(resume_score, 1.0)

    # -------------------------
    # 2) EMAIL CHECK (after resume)
    # -------------------------
    if is_email(text):
        return "Email", "Email structure detected", 0.9

    # -------------------------
    # 3) SCORE OTHER DOCUMENT TYPES
    # -------------------------
    scores = {}
    reasons = {}
    ...
    for doc_type, data in DOCUMENT_PATTERNS.items():
        ...
        # -----------------------------------------
        # REPORT BOOST LOGIC (for construction/project/site reports)
        # -----------------------------------------
        if doc_type == "Report":
            report_indicators = [
                "project details", "site report", "project report",
...
async def classify_files(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        if ext == "pdf":
            ...
            text = extract_text_from_image(raw)  # fallback
